...
>用`laravel/lumen`感觉还挺好用的，但是发现有点慢。执行到开始执行controller里的代码前 本机测试大约需要`60ms`左右。本来只打算写一个路由的，后来还是做成了个简易框架。VicR路由支持分组、中间件、缓存，路由规则支持{id}，{xxx}，`正则表达式`，本机测试路由消耗时间`1ms`以内。路由第一次执行后会缓存。
...
Router::get('/user/home',['use' => 'User@getUserInfoByName','as' => 'home','middle' => ['App@csrf']]);
```
#### resource
...
第一个参数数组 array(
	...
	'cache' => [ //两个参数 二选一 
		'time' => 60 //过期时间 单位秒
		'call' => Closure 函数方法 自定义缓存方法 调用时会带上（方法名称+请求参数）
	],
	'namespace' => '命名空间',
	'prefix' => '请求url的前缀'
)
